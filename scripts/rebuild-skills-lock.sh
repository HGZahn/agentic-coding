...
async function main() {
  ...
  for (const skill of skills) {
    ...
    if (!src) {
      throw new Error(`Missing source mapping for skill: ${skill}`);
    }
  ...
  const unknownMappings = Object.keys(sourceMap).filter((name) => !skills.includes(name));
  if (unknownMappings.length > 0) {
    console.error(`Warning: mappings exist for missing local skills: ${unknownMappings.join(', ')}`);
  }
